...
export function AdminCommissionerPanel() {
  ...
  function getStatus(invite: Invite) {
    if (invite.usedBy) return { label: "Used", color: "bg-green-500/20 text-green-400" };
    if (new Date(invite.expiresAt) < new Date()) return { label: "Expired", color: "bg-red-500/20 text-red-400" };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-bold text-white">Generate Commissioner Invite</h3>
        <div className="flex gap-3">
          <input
            ...
            className="flex-1 rounded-lg border border-white/20 bg-white/5 px-4 py-2 text-white placeholder:text-white/40 focus:border-[var(--slidey)] focus:outline-none"
          />
          <button
            onClick={handleGenerate}
            disabled={generating}
            className="rounded-lg bg-[var(--slidey)] px-6 py-2 text-sm font-semibold text-white hover:opacity-80 transition disabled:opacity-50"
          >
            {generating ? "..." : "Generate"}
          </button>
        </div>
          ...
          <div className="rounded-lg border border-[var(--slidey)]/30 bg-[var(--slidey)]/10 p-4 space-y-2">
            ...
            <div className="pt-2 border-t border-white/10">
              ...
              <button
                ...
                className="text-xs text-white/60 hover:text-white transition mt-1"
              >
                Copy message
              </button>
            </div>
      ...
      {invites.length > 0 && (
        <div className="space-y-3">
          <h3 className="text-lg font-bold text-white">Commissioner Invites</h3>
      ...
      {commissioners.length > 0 && (
        <div className="space-y-3">
          <h3 className="text-lg font-bold text-white">Active Commissioners</h3>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-xs text-white/50 uppercase tracking-wider">
