...
const PROSPECTS_2026 = [
  ...
  },
  {
    ...
    height: "6-2",
    weight: 195,
    notes: "The #1 WR in the class on many boards. One of the highest floors among true juniors thanks to strong hands and a consistently reliable track record. Route running and releases are already NFL-caliber. Dependable blocker. Complete receiver who should contribute immediately at the next level.",
  },
  {
    rank: 10,
    name: "Caleb Downs",
  ...
  },
  {
    ...
    height: "6-6",
    weight: 315,
    notes: "Elite offensive tackle prospect with prototypical size and length. Smooth mover in pass protection with strong anchor. Georgia's pipeline of NFL tackles continues. Projects as a day-one starter at left or right tackle. Top-25 overall prospect.",
  },
  {
    rank: 25,
    name: "Blake Miller",
    ...
    height: "6-6",
    weight: 310,
    notes: "Considered the 'most pro-ready' tackle in the class. Technically sound with consistent hand placement and footwork. May not have the highest ceiling but his floor is very high — projects as a reliable starter from day one.",
  },
  {
    rank: 26,
  ...
  },
  {
    ...
    height: "5-10",
    weight: 180,
    notes: "Transferred from USC to Georgia. Electric playmaker with game-breaking speed and return ability. Dynamic after the catch with elite agility. Undersized but plays bigger than his frame. Potential top-30 pick as a speed/slot weapon.",
  },
  {
    rank: 31,
    name: "Peter Woods",
...
async function seedProspects2026() {
  console.log("🏈 Updating prospect pool to 2026 NFL Draft class (ranked by consensus big board)...\n");
  ...
  for (const row of referencedInPicks.rows) referencedIds.add(row.player_id as string);
  for (const row of referencedInResults.rows) referencedIds.add(row.player_id as string);

  if (referencedIds.size > 0) {
    console.log(`  ⚠ ${referencedIds.size} players referenced in picks/results — keeping those rows.`);
  } else {
  ...
  const inserted = await db
    ...
    .values(PROSPECTS_2026)
    .onConflictDoNothing()
    .returning();

  console.log(`  ✓ ${inserted.length} prospects inserted (ranked 1-${inserted.length})`);

  const positions: Record<string, number> = {};
  for (const p of PROSPECTS_2026) positions[p.position] = (positions[p.position] || 0) + 1;
  console.log("\n  Position breakdown:");
